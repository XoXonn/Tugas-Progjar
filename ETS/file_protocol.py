...
class FileProtocol:
    ...
    def proses_string(self, string_datamasuk=''):
        try:
            # Memisahkan perintah (kata pertama) dari sisa string
            parts = string_datamasuk.strip().split(' ', 1)
            command = parts[0].lower()
            
            params = []
            if len(parts) > 1:
                # Untuk UPLOAD, pisahkan nama file (kata pertama) dari data base64
                if command == 'upload':
            ...
            # Panggil method di FileInterface (upload, get, list)
